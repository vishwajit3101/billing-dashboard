...
# Defaults for FullEnrich
# Usage URL: https://app.fullenrich.com/api/v1/account/credits
# Standard credits for a basic account is often 50
FULLENRICH_TOTAL_DEFAULT = 50.0
...
FULLENRICH_TOTAL_CREDITS = float(os.getenv("FULLENRICH_TOTAL_CREDITS", FULLENRICH_TOTAL_DEFAULT))

def get_fullenrich_remaining_credits() -> tuple:
    # Always refresh from env in case it changed
    total_default = float(os.getenv("FULLENRICH_TOTAL_CREDITS", FULLENRICH_TOTAL_DEFAULT))
    
    if not FULLENRICH_API_KEY:
        print("[FullEnrich] No API key in .env → returning 0.0 remaining")
        return 0.0, total_default
    ...
    try:
        ...
        remaining = data.get("balance", data.get("credits_remaining", data.get("remaining", 0.0)))
        ...
        print(f"[FullEnrich] Error: {str(e)} → returning 0.0 remaining")
        return 0.0, total_default