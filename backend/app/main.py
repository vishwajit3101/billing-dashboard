...
async def get_dashboard(days: int = Query(30, ge=1, le=90)):
    ...
    for row in tools_rows:
        ...
        # Ensure history is sorted and has labels (posthog.py already does this now)
        # but if we want strictly 7 days for the detail panels of non-main tools:
    ...
    aws = {
        ...
        "weekly_change": aws_data.get("weekly_change", 0.0), # Maintain for card spiked logic if any
        "filtered_days": days
    }
